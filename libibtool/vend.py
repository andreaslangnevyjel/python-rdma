...
class MlxGeneralInfo(rdma.binstruct.BinStruct):
    """None (section None)"""
    __slots__ = (
    'reserved_0', 'hw_revision', 'device_id', 'reserved_96', 'uptime', 'reserved_320', 'fw_major', 'fw_minor',
    'fw_sub_minor', 'fw_build_id', 'fw_month', 'fw_day', 'fw_year', 'reserved_416', 'fw_hour', 'fw_psid',
    ...
    MEMBERS = [('reserved_0', 64, 1), ('hw_revision', 16, 1), ('device_id', 16, 1), ('reserved_96', 192, 1),
               ('uptime', 32, 1), ('reserved_320', 8, 1), ('fw_major', 8, 1), ('fw_minor', 8, 1),
               ('fw_sub_minor', 8, 1), ('fw_build_id', 32, 1), ('fw_month', 8, 1), ('fw_day', 8, 1), ('fw_year', 16, 1),
               ('reserved_416', 16, 1), ('fw_hour', 16, 1), ('fw_psid', 128, 1), ('fw_init_file_version', 32, 1),
               ('reserved_608', 8, 1), ('hw_major', 8, 1), ('hw_minor', 8, 1), ('hw_sub_minor', 8, 1),
               ('reserved_640', 224, 1)]
    ...
    def pack_into(self, buffer, offset=0):
        ...
        struct.pack_into('>LBBBBLBBHHH', buffer, offset + 36, self.uptime, self.reserved_320, self.fw_major,
                         self.fw_minor, self.fw_sub_minor, self.fw_build_id, self.fw_month, self.fw_day, self.fw_year,
                         self.reserved_416, self.fw_hour)
        struct.pack_into('>LBBBB', buffer, offset + 72, self.fw_init_file_version, self.reserved_608, self.hw_major,
                         self.hw_minor, self.hw_sub_minor)
    ...
    def unpack_from(self, buffer, offset=0):
        ...
        (self.uptime, self.reserved_320, self.fw_major, self.fw_minor, self.fw_sub_minor, self.fw_build_id,
         self.fw_month, self.fw_day, self.fw_year, self.reserved_416, self.fw_hour,) = struct.unpack_from(
            '>LBBBBLBBHHH', buffer, offset + 36)
        (self.fw_init_file_version, self.reserved_608, self.hw_major, self.hw_minor,
         self.hw_sub_minor,) = struct.unpack_from('>LBBBB', buffer, offset + 72)
